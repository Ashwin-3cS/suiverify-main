...
const PANUploadStep: React.FC<PANUploadStepProps> = ({
  ...
  const handlePANUpload = async (file: File) => {
    ...
    try {
      logger.log(" Processing PAN file:", file.name, file.size, "bytes");
      ...
      logger.log(" Calling backend API to extract PAN data...");
      ...
      logger.log(" Backend API returned extracted PAN data");
      ...
      if (result.data) {
        ...
        logger.log(" Complete PAN data prepared:", {
  ...
  const handleEditSave = async (editedData: PANData) => {
    ...
    try {
      ...
      logger.log(" Sending corrected PAN data to backend...");
