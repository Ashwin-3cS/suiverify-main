...
export default function ZkLoginTransactionTest() {
    ...
    const handleExecuteTransaction = async () => {
        try {
            ...
            if (!cached || !cached.address) {
                throw new Error("No zkLogin session found. Please sign in with zkLogin first.");
            }
            ...
            if (!receiverAddress || receiverAddress.length !== 66 || !receiverAddress.startsWith("0x")) {
                throw new Error("Please enter a valid Sui address (starts with 0x, 66 characters)");
            }
    ...
    return (
        ...
        >
            ...
            {txDigest && (
                ...
                >
                    ...
                    <div className="flex items-center justify-between gap-2">
                        <p className="font-mono text-xs break-all" style={{ color: colors.lightBlue }}>
                            {txDigest}
                        </p>
                        <button
            ...
            {userAddress && (
                <div className="mb-4">
                    <label className="block text-xs font-semibold mb-2" style={{ color: colors.lightBlue }}>
                        Your zkLogin Address
                    </label>
                    <div
                        className="px-4 py-2 rounded-lg font-mono text-xs"
                        style={{ backgroundColor: `${colors.primary}10`, color: colors.white }}
                    >
                        {userAddress}
                    </div>
                </div>
            ...
            {!txDigest && (
                <div className="space-y-4">
                    <div>
                        <label className="block text-sm font-semibold mb-2" style={{ color: colors.white }}>
                            Receiver Address
                        </label>
                        <input
                            type="text"
                    ...
                    >
                        ...
                        <p className="text-xs mt-2">Get from Discord: <code>!faucet {userAddress || 'YOUR_ADDRESS'}</code></p>
                    </div>
