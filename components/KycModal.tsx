...
const KycModal: React.FC<KycModalProps> = ({
  ...
  const handleAadhaarUpload = async (file: File) => {
    ...
    try {
      ...
      const result = await handleApiCall(
        buildApiUrl(API_ENDPOINTS.EXTRACT_AADHAAR_DATA),
        formData,
      );
      ...
      if (result.success && result.data) {
        ...
        setError(
          result.message ||
            "Failed to process Aadhaar image. Please ensure the image is clear and readable.",
        );
      ...
      const errorMsg =
        err instanceof Error
          ? err.message
          : "An error occurred while processing the Aadhaar image";
      setError(errorMsg);
  ...
  const handleFaceVerification = async (capturedImage: string) => {
    ...
    try {
      ...
      if (!aadhaarData?.phone_number) {
        setError(
          "Phone number not found in Aadhaar data. Cannot proceed with verification.",
        );
        return;
      }
      ...
      if (result.success && result.data) {
        ...
        if (result.data.match) {
          ...
          const failureMessage =
            result.data.message ||
            "Face verification failed. The faces do not match with sufficient confidence.";
  ...
  const generateOtp = async () => {
    ...
    try {
      ...
      const result = await handleApiCall(
        buildApiUrl(API_ENDPOINTS.GENERATE_OTP),
        formData,
      );
      ...
      if (result.success) {
        ...
        setError(
          result.message ||
            "Failed to generate OTP. Please check the phone number.",
        );
  ...
  const handleOtpVerification = async () => {
    ...
    try {
      ...
      const result = await handleApiCall(
        buildApiUrl(API_ENDPOINTS.VERIFY_OTP_ENDPOINT),
        formData,
      );
    ...
    <div className="fixed inset-0 bg-primary/40 bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-[#F8FAFC] rounded-2xl md:p-8 p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl relative border-[3px] border-primary outfit">
            ...
            <div className="flex items-center">
              ...
              <div
                className={`w-16 h-1 mx-2 ${
                  ["face", "generate-otp", "verify-otp", "complete"].includes(
                    step,
                  )
                    ? "bg-success"
                    : "bg-light-gray"
              ...
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-medium ${
                  step === "face"
                    ? "bg-primary"
                    : ["generate-otp", "verify-otp", "complete"].includes(step)
                      ? "bg-success"
                      : "bg-light-gray"
                }`}
              >
              ...
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-medium ${
                  step === "generate-otp"
                    ? "bg-primary"
                    : ["verify-otp", "complete"].includes(step)
                      ? "bg-success"
                      : "bg-light-gray"
                }`}
              >
        ...
        <div>
          ...
          {step === "aadhaar" && (
            <div>
              <h3 className="text-lg font-semibold text-charcoal-text mb-4">
                Upload Aadhaar Card
              </h3>
              ...
              {!previewUrl ? (
                <div className="border-2 border-dashed border-primary/30 rounded-lg p-8 text-center">
                  ...
                  <p className="text-charcoal-text/70 mb-6">
                    Choose a clear image of your Aadhaar card
                  </p>
              ...
              {faceResult && (
                ...
                >
                  ...
                  <div className="text-sm space-y-1">
                    ...
                    <p className="text-charcoal-text">{faceResult.message}</p>
                  </div>
                </div>
              )}

              <div className="text-center mb-6">
                <h4 className="text-lg font-semibold text-charcoal-text mb-2">
                  Live Face Capture
                </h4>
                ...
                {!faceImage ? (
                  <LivenessWebcam
                    onVerified={(capturedImage) => {
                      ...
                      handleFaceVerification(capturedImage);
                    }}
                    onError={(err) => {
                      setError(err);
                      toast.error(err);
                    ...
                    {isProcessing && (
                      <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center rounded-lg">
                        <div className="text-center text-white">
                          ...
                          <p className="text-sm">
                            Verifying face against Aadhaar...
                          </p>
                        </div>
                      </div>
                    )}
              ...
              <div className="flex gap-4">
                {faceImage && !isProcessing && (
                  <>
                    ...
                    <Button
                      onClick={() =>
                        faceResult?.match && setStep("generate-otp")
                      }
                      variant="success"
                      disabled={!faceResult?.match}
                      className="flex-1"
                    >
                      Next: Generate OTP
                    </Button>
          ...
          {step === "generate-otp" && (
            <div>
              ...
              <div className="text-center mb-6">
                ...
                <p className="text-lg font-semibold text-charcoal-text mb-6">
                  {phoneNumber}
                </p>
